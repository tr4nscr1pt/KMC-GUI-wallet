...
// Copyright (c) 2016-2017 The KMCoin developers
...
void SendFrame::amountValueChange() {
    ...
    Q_FOREACH (TransferFrame * transfer, m_transfers) {
      ...
      float donationpercent = amount * 0.05 / 100; // donation is 0.05%
...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("kriptomarka://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "kriptomarka:");
    }
    if(!_request.startsWith("kriptomarka:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with kriptomarka:"), QtCriticalMsg));
      return;
    }
...
void SendFrame::sendClicked() {
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
	 ...
	 {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "KoQCZFRJ1Dh18HBp56oKyzj1inCUWvUABXyhDsP3aoFUV6yzqUWbhTZTd4aaixKmhdPAEgvA8KNZ9Wq9rtHrvM2KA7AGFuF";
